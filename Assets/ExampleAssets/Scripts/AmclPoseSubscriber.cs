...
namespace RosSharp.RosBridgeClient
{
    ...
    : UnitySubscriber<MessageTypes.Geometry.PoseWithCovarianceStamped>
    {
        ...
        public new void Update()
        {
            ...
            {
                PublishedTransform.gameObject.layer = LayerMask.NameToLayer("666");//AR相机不可见
            ...
            {
                //实体机需要注释掉这句
                // PublishedTransform.gameObject.layer = LayerMask.NameToLayer("Default");

                if (ar_y > 0)
                {
                    ar_y = ARPositionController.position.y; //获取平面高度
                    position.y = ar_y;
                    ProcessMessage();//以初始位姿显示
                }
